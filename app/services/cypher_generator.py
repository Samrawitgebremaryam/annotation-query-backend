...
class CypherQueryGenerator(QueryGeneratorInterface):
    ...
    def process_result_graph(self, results, graph_components):
        ...
        # Get named types and identifier fields from schema for all node types in the results
        named_types = set()
        identifier_fields = {}
        for record in results:
            for item in record.values():
                if isinstance(item, neo4j.graph.Node):
                    ...
                    identifier_field = self.schema_manager.get_identifier_field(
                        node_type
                    )
                    if identifier_field:
                        identifier_fields[node_type] = identifier_field

        for record in results:
            for item in record.values():
                if isinstance(item, neo4j.graph.Node):
                    node_type = list(item.labels)[0]
                    identifier_field = identifier_fields.get(node_type, "id")
                    node_id = f"{node_type} {item[identifier_field]}"
                    ...
                    if node_id not in node_dict:
                        ...
                        # Add all properties based on schema
                        node_schema = self.schema_manager.get_node_type(node_type)
                        for prop_name, prop_config in node_schema.get(
                            "properties", {}
                        ).items():
                            if prop_name in item and prop_name != identifier_field:
                                if graph_components["properties"]:
                                    if prop_name != "synonyms":
                                        node_data["data"][prop_name] = item[prop_name]
                                else:
                                    if prop_name in named_types:
                                        node_data["data"]["name"] = item[prop_name]

                        # Set display name if not already set
                        if "name" not in node_data["data"]:
                            display_field = self.schema_manager.get_display_field(
                                node_type
                            )
                            if display_field and display_field in item:
                                node_data["data"]["name"] = item[display_field]
                            else:
                    ...
                    source_type = list(item.start_node.labels)[0]
                    target_type = list(item.end_node.labels)[0]

                    # Get identifier fields for source and target
                    source_id_field = identifier_fields.get(source_type, "id")
                    target_id_field = identifier_fields.get(target_type, "id")

                    source_id = f"{source_type} {item.start_node[source_id_field]}"
                    target_id = f"{target_type} {item.end_node[target_id_field]}"

                    edge_data = {
                        "data": {
                            "edge_id": f"{source_type}_{item.type}_{target_type}",
                            "label": item.type,
                            "source": source_id,
                            "target": target_id,
                        }
                    ...
                    rel_schema = self.schema_manager.get_relationship_type(item.type)
                    for prop_name, prop_config in rel_schema.get(
                        "properties", {}
                    ).items():
                        if prop_name in item:
                            if prop_name == "source":
                                edge_data["data"]["source_data"] = item[prop_name]
                            else:
                                edge_data["data"][prop_name] = item[prop_name]

                    edges.append(edge_data)
                    if item.type not in edge_type:
                        edge_type.add(item.type)
                        edge_to_dict[item.type] = []
                    edge_to_dict[item.type].append(edge_data)
    ...
    def parse_id(self, request):
        nodes = request["nodes"]
        # Get named types and prefixes from schema manager
        named_types = {}
        prefixes = []
        ...
        for node in nodes:
            ...
            # Get display field for this node type
            display_field = self.schema_manager.get_display_field(node_type)
            if display_field:
                named_types[node_type] = display_field

            # Get identifier field for this node type
            identifier_field = self.schema_manager.get_identifier_field(node_type)
            if identifier_field:
                # Add common prefixes for this type if they exist in schema
        ...
        for node in nodes:
            ...
            id = node["id"].lower()
            is_name_as_id = all(
                not id.startswith(prefix.lower()) for prefix in prefixes
            )
            no_id = node["id"] != ""

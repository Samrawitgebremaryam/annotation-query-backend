...
class MeTTa_Query_Generator(QueryGeneratorInterface):
    ...
    async def query_Generator(self, data, node_map):
        nodes = data["nodes"]
        ...
        if "predicates" not in data:
            ...
            node_without_predicate = [
                node for node in nodes if node["node_id"] not in node_with_predicate
            ]

        # if there is no predicate
        if "predicates" not in data or (
            node_without_predicate is not None and len(node_without_predicate) != 0
        ):
            for node in node_without_predicate:
                ...
                if node["id"]:
                    ...
                    if len(node["properties"]) == 0:
                        ...
                        metta_output += self.construct_node_representation(
    ...
    def metta_seralizer(self, metta_result):
        ...
        for node in metta_result:
            ...
            for metta_symbol in node:
                if (
                    isinstance(metta_symbol, SymbolAtom)
                    and metta_symbol.get_name() == ","
                ):
                    continue
    ...
    def parse_id(self, request):
        ...
        for node in nodes:
            ...
            is_name_as_id = all(
                not node["id"].startswith(prefix) for prefix in prefixes
            )
            no_id = node["id"] != ""

...
def get_graph_storage_dir():
    # Get the project root directory (where the app directory is)
    project_root = Path(__file__).parent.parent.parent
    # Define the graph storage directory
    graph_dir = project_root / "storage" / "graph"
...
def generate_result(query_code, annotation_id, requests, result_status, status=None):
    try:
        ...
        if get_status(annotation_id) == TaskStatus.CANCELLED.value:
            socketio.emit(
                "update",
                {"status": TaskStatus.CANCELLED.value, "update": {"graph": False}},
            )
            status = update_task(annotation_id)
            result_status.set()
            return
        ...
        # Get the graph storage directory and create file path
        graph_dir = get_graph_storage_dir()
        file_path = graph_dir / f"{annotation_id}.json"

        # Ensure parent directory exists and save file
        file_path.parent.mkdir(parents=True, exist_ok=True)
        with open(file_path, "w") as file:
            json.dump(grouped_graph, file)

        AnnotationStorageService.update(
            annotation_id, {"path_url": str(file_path.resolve())}
        )
        ...
        status = update_task(
            annotation_id,
            {"nodes": grouped_graph["nodes"], "edges": grouped_graph["edges"]},
        )
        socketio.emit(
            "update",
            {"status": status, "update": {"graph": True}},
            to=str(annotation_id),
        )
        ...
        set_status(annotation_id, TaskStatus.CANCELLED.value)
        update_task(annotation_id, {"nodes": [], "edges": []})
        socketio.emit(
            "update", {"status": TaskStatus.CANCELLED.value, "update": {"graph": False}}
        )
        result_status.set()
        logging.error("Error generating result graph %s", e)
    except Exception as e:
        ...
        socketio.emit(
            "update", {"status": TaskStatus.FAILED.value, "update": {"graph": False}}
        )
        AnnotationStorageService.update(
...
def start_thread(annotation_id, args):
    ...
    def send_annotation():
        try:
            generate_result(
                find_query, annotation_id, request, all_status["result_done"]
            )
        except Exception as e:
            logging.error("Error generating result graph %s", e)
    ...
    summary_generator = threading.Thread(name="summmary_generator", target=send_summary)

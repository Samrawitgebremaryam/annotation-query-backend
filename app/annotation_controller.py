...
def handle_client_request(query, request, node_types):
    ...
    if annotation_id:
        existing_query = AnnotationStorageService.get_query(annotation_id, query[0])
    else:
        existing_query = None
...
def process_full_data(annotation_id):
    ...
    try:
        file_path = generate_file_path(file_name=title, extension="xls")
        ...
        file_path = convert_to_csv(parsed_result, file_name=title)
        file_path = adjust_file_path(file_path)
